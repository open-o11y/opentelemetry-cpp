...
public:
   ...
   * @param timeout that the forceflush should finish within. A default timeout of 0 mean no timeout is applied. 
   * If this timeout is exceeded without completing the ForceFlush, the method will be aborted. 
   * @return a result code indicating whether it succeeded, failed or timed out 
  */
  virtual ProcessorResult ForceFlush(
   ...
   * After the call to Shutdown, subsequent calls to OnReceive, ForceFlush will immediately return.
   * @param timeout that the shutdown should finish within. A default timeout of 0 mean no timeout is applied. 
   * @return a ShutDown result (if it succeeded, failed or timed out) 
   */
  virtual ProcessorResult Shutdown(
      std::chrono::microseconds timeout = std::chrono::microseconds(0)) noexcept = 0;
};
}  // namespace logs
}  // namespace sdk
