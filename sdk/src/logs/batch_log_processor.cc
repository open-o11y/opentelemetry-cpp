...
void BatchLogProcessor::OnReceive(std::unique_ptr<LogRecord> &&record) noexcept
{
  ...
  // If the ciruclar buffer is half full,
  // signal the worker thread to start a new export cycle.
...
void BatchLogProcessor::ForceFlush(std::chrono::microseconds timeout) noexcept
{
  if (is_shutdown_.load() == true)
  {
    return;
...
void BatchLogProcessor::Shutdown(std::chrono::microseconds timeout) noexcept
{
  is_shutdown_ = true;
